...
def checkout():
    ...
    if form.validate_on_submit():
        ...
        infor = Infor(name=form.name.data, address=form.address.data, country=country, city=form.city.data,
                      postcode=form.postcode.data, phone=form.phone.data, total_price=total)
        ...
        session.pop('order')
        return redirect(url_for('home'))
    return render_template('checkout.html', title='Check Out', form=form, total=total, subtotal=subtotal,
                           shipping=shipping)
